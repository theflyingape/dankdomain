...
function newSession(ev) {
    let options: ITerminalOptions = {
        allowProposedApi: true, scrollSensitivity: 5, smoothScrollDuration: 250,
    ...
    if (ev == 'Logon') {
        ...
        fetch(`${app}/player/?cols=${term.cols}&rows=${term.rows}`, { method: 'POST' }).then(function (res) {
            ...
            setTimeout(() => window.dispatchEvent(new Event('resize')), 1440)
        })
    }
    else {
        term.focus()
...
<span style="color:brown; font-size:smaller;">🇺🇸 &copy; 2017 - 2024 <a href="https://robert.hurst-ri.us" target="_new">Robert Hurst</a> 🧙</span><br><br>
